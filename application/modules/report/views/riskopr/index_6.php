...
<h3>รายงานข้อมูลแผนการปฏิบัติการและรายงานผลการบริหารจัดการความเสี่ยง  รอบ  1 ตุลาคม– 31 มี ค. (6 เดือน) </h3>
...
<div id="search">
  <div id="searchBox">
  	...
  	<input type="hidden" name="permis" id="permis" value="<?=$rs['permis']?>" />
		<? if(@$rs['permis']=='on'){ ?>
			<span>ปีงบประมาณ : </span><?=form_dropdown('year_data',get_year_option(),@$_GET['year_data'],'','แสดงทุกปี');?>
			<br />
			<span> ภาควิชา : </span>
		...
		<span class="sp_process">
			...
			if(@$rs['permis']=='on'){
				...
				$missionid = @$_GET['missionid'];
					...
					echo form_dropdown('process',get_option('process as id','process as title','risk_est_head'," process IN (SELECT DISTINCT process FROM risk_est WHERE year_data='".$year_data."' and sectionid='".$sectionid."' and objectiveid_1='".$objectiveid_1."' and objectiveid_2='".$objectiveid_2."' and objective_3 ='".$objective_3."' and missionid ='".$missionid."') order by title "),@$_GET['process'],'','--เลือกกระบวนงาน--');
			?>
