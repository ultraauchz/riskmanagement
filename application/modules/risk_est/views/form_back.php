...
$(function(){
	...
	$(".btn_add_control_risk").click(function(){
		...
		//alert(num);
		var l = $('input[name=control_row['+num+'] ]').val();
		var newrow = '<tr>';
	    newrow += '<td>'
	    newrow += '<input name="risk_control_row['+num+']" type="hidden" value="'+num+'" />';
	    newrow += '<input name="control_row['+num+']" type="hidden" value="1" />';
	    newrow += '<span><br />';
		newrow += '<input name="control_risk'+num+'[1]" type="text" class="" value="" style="width:350px;" />';
		newrow += ' <a href="'+num+'" onclick="return false;"class="btn btn-warning add_control_risk_sub1"><i class="icon-plus-sign"></i></a>';
		newrow += '<span class="control_risk_sub1'+num+'"></span>';		
		newrow += '</span></td>';
		
	    newrow +='<td><input name="estimate_control_row['+num+']" type="hidden" value="1" />';
	    newrow +='<textarea name="estimate_control_risk'+num+'[1]" class="" rows="4" style="width:650px"></textarea>';
	    newrow +=' <a href="'+num+'" onclick="return false;"class="btn btn-warning add_control_risk_sub2"><i class="icon-plus-sign"></i></a>'
	    newrow += '<span class="estimate_control_risk_sub2'+num+'"></span></td>';
		...
		$("[name=control_risk"+num+"[1]]").rules( 'add', {required: true , messages: {required: "กรุณาระบุการควบคุมที่มีอยู่"}});
		$("[name=estimate_control_risk"+num+"[1]]").rules( 'add', {required: true , messages: {required: "กรุณาระบุการประเมินการควบคุมที่มีอยู่"}});
		...
		$(".add_control_risk_sub1").live("click",function(){
			//alert(11);
			var i = $(this).attr('href');
			//alert(i);
			var l = $('input[name=control_row['+i+'] ]').val();
			var n = $('input[name=control_risk['+i+'] ]').val();
			var num =  parseInt(l)+1;
			var newrow = '<span><br />';
				newrow += '<input name="control_risk'+i+'['+num+']" type="text" class="" value="" style="width:350px;" />';
				newrow += ' <a href="#" onclick="return false;" class="btn btn-danger btn_delete_control_risk_sub1"><i class="icon-trash"></i></a>';
				newrow += '</span>';
		    		
			$('.control_risk_sub1'+i).before(newrow);
			//alert(i);
			$("[name=control_risk"+i+"["+num+"] ]").rules( 'add', {required: true , messages: {required: "กรุณาระบุการควบคุมที่มีอยู่"}});
			$('input[name=control_row['+i+']]').val(num);
			validate_form();
		})
		
		$(".add_control_risk_sub2").live("click",function(){
			//alert(11);
			var i = $(this).attr('href');
			//alert(i);
			var l = $('input[name=estimate_control_row['+i+'] ]').val();
			var n = $('input[name=estimate_control_risk['+i+'] ]').val();
			var num =  parseInt(l)+1;
			var newrow = '<span><br />';
				newrow +='<td><textarea name="estimate_control_risk'+i+'['+num+']" class="" rows="4" style="width:650px"></textarea>';
				newrow += ' <a href="#" onclick="return false;" class="btn btn-danger btn_delete_control_risk_sub2"><i class="icon-trash"></i></a>';
				newrow += '</span>';
		    		
			$('.estimate_control_risk_sub2'+i).before(newrow);
			//alert(i);
			$("[name=estimate_control_risk"+i+"["+num+"] ]").rules( 'add', {required: true , messages: {required: "กรุณาระบุการควบคุมที่มีอยู่"}});
			$('input[name=estimate_control_row['+i+']]').val(num);
			validate_form();
		})
	...
	$(".btn_delete_control_risk_sub1").live("click",function(){
		if(confirm('ลบรายการนี้ ?')){
		  	  $(this).closest("span").remove();
		}
	})
	$(".btn_delete_control_risk_sub2").live("click",function(){
		if(confirm('ลบรายการนี้ ?')){
		  	  $(this).closest("span").remove();
		}
	})
...
$(document).ready(function(){
		...
		var ii = $('input[name=num_i]').val();
		 ...
		 for(jj=1;jj<=ii;jj++){
		 	$("[name=control_risk"+jj+"["+jj+"]]").rules( 'add', {required: true , messages: {required: "กรุณาระบุการควบคุมที่มีอยู่"}});
		$("[name=estimate_control_risk"+jj+"["+jj+"]]").rules( 'add', {required: true , messages: {required: "กรุณาระบุการประเมินการควบคุมที่มีอยู่"}});
		}
...
<fieldset>
	...
	<table class="table table-form table-bordered table-striped table-horizontal">	  
	  <tr>
	    <th width="420px">การควบคุมที่มีอยู่</th>
	  ...
	  <?   $control_i = 1;
	   ...
	   ?>
	  		<tr>
	   		 <td>
	   		 	<input name="risk_control_row[<?=$control_i?>]" type="hidden" value="<?=$control_i?>" />
	   		 	<input name="control_row[<?=$control_i?>]" type="hidden" value="1" />
	   		 	<input name="control_risk<?=$control_i?>[<?=$control_i?>]" style="width:350px;" type="text" class="" value="<?=@$com_risk['control_risk']?>" />
	   		 	<a href="<?=$control_i?>" onclick="return false;" class="btn btn-warning btn_add_control_risk" ><i class="icon-plus-sign"></i></a>
	   		 	<span class="control_risk_sub1<?=$control_i?>"></span>
	   		 </td>
	   		 <td>
	   		 	<input name="estimate_control_row[<?=$control_i?>]" type="hidden" value="1" />
	   		 	<textarea name="estimate_control_risk<?=$control_i?>[<?=$control_i?>][1]" class="" rows="4" style="width:650px"><?=@$com_risk['estimate_control_risk']?></textarea>
	   		 	<a href="<?=$control_i?>" onclick="return false;"class="btn btn-warning add_control_risk_sub2"><i class="icon-plus-sign"></i></a>
	   		 	<span class="estimate_control_risk_sub2<?=$control_i?>"></span>
	   		 </td>
	    	 <td style="width:80px;text-align:center;">
	  ...
	  <tr>
	    <td>
	    	<input name="risk_control_row[<?=$control_i?>]" type="hidden" value="<?=$control_i?>" />
	    	<input name="control_row[<?=$control_i?>]" type="hidden" value="1" />
	    	<input name="control_risk<?=$control_i?>[<?=$control_i?>]" style="width:350px;" type="text" class="" value="" />
	    	<a href="<?=$control_i?>" onclick="return false;"class="btn btn-warning add_control_risk_sub1"><i class="icon-plus-sign"></i></a>
	    	<span class="control_risk_sub1<?=$control_i?>"></span>
	    </td>
	    <td>
	    	<input name="estimate_control_row[<?=$control_i?>]" type="hidden" value="1" />
	    	<textarea name="estimate_control_risk<?=$control_i?>[<?=$control_i?>]" class="" rows="4" style="width:650px"></textarea>
	    	<a href="<?=$control_i?>" onclick="return false;"class="btn btn-warning add_control_risk_sub2"><i class="icon-plus-sign"></i></a>
	   		<span class="estimate_control_risk_sub2<?=$control_i?>"></span>
	    </td>
	    <td style="width:80px;text-align:center;">
...
<table class="table table-form table-bordered table-striped table-horizontal">
  ...
  <tr>
    ...
    <td><input name="owner_risk" type="text" class="" style="width:350px" value="<?=@$rs['owner_risk'];?>" /></td>
  </tr>
  <tr>
    <th width="400px">ตำแหน่ง</th>
    <td><input name="owner_risk_position" type="text" style="width:350px" class="" value="<?=@$rs['owner_risk_position'];?>" /></td>
  </tr>
  <tr>
