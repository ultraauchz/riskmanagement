...
<? 
$admin_system = $this->db->getarray("SELECT * FROM admin_menu where pid = 0 and show_menu = 1 ORDER BY order_no ");	
foreach($admin_system as $system):
	$url = $system['url']!='' ? $system['url'] : '#';
	echo '<li><a href="'.$url.'">'.$system['title']."</a>";
	$admin_menu = $this->db->getarray("SELECT * FROM admin_menu WHERE pid=".$system['id']." and show_menu = 1 order by order_no ");
